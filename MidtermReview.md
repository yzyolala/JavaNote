...
是在一个类里面，方法名字必须相同，而参数或参数列表必须不同，返回类型和访问修饰符可以相同也可以不同

方法能够在同一个类中或者在一个子类中被重载

重写：

不能对方法名、参数列表和返回值类型进行修改

访问修饰符不能比父类更严格（父类的一个方法被声明为 public，那么在子类中重写该方法就不能声明为 protected）
